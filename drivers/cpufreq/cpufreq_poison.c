...
#ifdef USES_POISON_TUNEABLES_AND_SHIT
/******************** Tunable parameters: ********************/
 ...
 */
#define DEFAULT_SLEEP_WAKEUP_FREQ 936000
...
static unsigned int dt2w_regions;
...
static ssize_t show_dt2w_regions(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", dt2w_regions);
}

static ssize_t store_dt2w_regions(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
...
define_global_rw_attr(dt2w_regions);
